...
class turtlebot_hategi():
    ######## CHANGE THE FOLLOWING VALUES #########
    server_public_dns = 'http://ec2-54-180-142-89.ap-northeast-2.compute.amazonaws.com' #must start with http:// .  Don't include a trailing "/"
    ...
    def __init__(self):
	#initialize ros node
        rospy.init_node('turtlebot_hategi', anonymous=False)
	...
	if(data["status"] == "pending"): #someone is pending coffee!  Oh ya... let's get moving
		...
		if not success:
            ...
            # 실패 또는 성공시에 update
			data = json.load(urllib2.urlopen(self.server_public_dns + "/Hategi-server/escort_queue.php?update&id=" + data["id"] + "&status=failed"))
    		else:
			# We made it!
			...
			if state == GoalStatus.SUCCEEDED:
			    ...
			    self.count_no_one_needs_escort_in_a_row = 0 #reset to 0
    ...
    def INeedPower(self):
    #도킹스테이션에서 충전중인가 확인 후, 완충 상태가 아닐때만 충전을 계속함
...
if __name__ == '__main__':
	...
	while(hategi.deliver_escort() and not rospy.is_shutdown()):
